...
probafiles = [Fig_dir2+'probamap_2002_b_g_dor_n_d_n3_md30_ne100.pkl',
			  ...
			  Fig_dir2+'probamap_2009_b_g_r_d_n3_md30_ne100.pkl',
...
#############################################################################
Models, Model_scenes, Model_times, Sepvals, Existence = pickle.load( open( Fig_dir2 + '/5Modelsdata_b_g_r_d_q5_n3_md30_ne100.pkl', "rb" ) )

pred_times = pickle.load( open( Fig_dir2 + '/predtimes.pkl', "rb" ) )

Proba = pickle.load( open( Fig_dir2+'probamap_2009_b_g_r_d_n3_md30_ne100.pkl', "rb" ) )
Ccor = pickle.load( open( Fig_dir2+'Ccor_all.pkl', "rb" ) )

Eph = pickle.load( open( Fig_dir2+'Ephemeral.pkl', "rb" ) )

#SF2 = pickle.load( open( Fig_dir2+'SF2_all.pkl', "rb" ) )

#seasons_4,seasons_2 = pickle.load( open( Fig_dir2+'seasons.pkl', "rb" ) )
#U_2 = np.unique(seasons_2)

#Long = pickle.load( open( Fig_dir2+'Longevity_wks_all_c.pkl', "rb" ) )

#SPM = pickle.load( open( Fig_dir2 + '/SPMx10_all.pkl', "rb" ) )
...
Diffmap = np.zeros(np.asarray(Proba).shape, dtype = np.int8)
...
for num in range(len(prediction_scenes)):
	scene = prediction_scenes[num]

	idx = np.where(np.asarray(prediction_scenes) == scene)[0][0]
	time = pred_times[idx]
	#scene_season_4, scene_season_2 = fn.count_seasons([time])
	#Season_idx = np.where(seasons_2 == scene_season_2)
	#SF2_idx = np.where(U_2 == scene_season_2)[0][0]

	#print (testidx, SF_2_idx, Season_idx)

	#pred = Ccor[num]
	proba = Proba[num]
	pred = fn.proba2pred(proba, Sepvals[2], zones, mask)
	E = Eph[num]
	#L = Long[num]
	...
	if scene in Model_scenes+Inletscenes+Digiscenes:
		...
		if scene in Model_scenes:
			...
			edge_w = 1; mark = 'x'; exist_kso = 1




		diffmap, S, P, A, QFN , QFP = fn.make_zonewise_perf(pred, proba, kso, zones)
		...
		ax.imshow(np.ma.masked_where(np.logical_or(mask!=0,diffmap <= 0), L), cmap = plt.cm.summer_r, vmin = 0, vmax = 55)
		ax.imshow(np.ma.masked_where(np.logical_or(mask!=0,diffmap >= 0), L), cmap = plt.cm.winter_r, vmin = 0, vmax = 55)

		ax.set_title(str(np.round(100*np.asarray(S),0))+' // '+str(np.round(100*np.asarray(P),0))+' // '+str(np.round(100*np.asarray(A),0)))
...
for i in range(len(Diffmap)):
	...
	ax.imshow(Diffmap[i], cmap = plt.cm.seismic, vmin = -2, vmax = 2)
...
pickle.dump( Diffmap, open( Fig_dir2 + 'Diffmap_cor.pkl', "wb" ) )
...
mapz = ax.imshow(np.ma.masked_where(mask!=0, FTU[130]/10), cmap = plt.cm.autumn_r, vmin = 0, vmax = 10)
