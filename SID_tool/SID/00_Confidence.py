...
#probafiles = [Fig_dir2+'probamap_2004_bgx.pkl', Fig_dir2+'probamap_2009_bgx.pkl',Fig_dir2+'probamap_2010_bgx.pkl',Fig_dir2+'probamap_2017_bgx.pkl']
#probafiles = [Fig_dir2+'probamap_2004_bdog.pkl', Fig_dir2+'probamap_2009_bdog.pkl',Fig_dir2+'probamap_2010_bdog.pkl',Fig_dir2+'probamap_2017_bdog.pkl']
...
#FTU = pickle.load( open( Fig_dir2 + '/FTUx10_all.pkl', "rb" ) )



###############################################################################
# Load the processed stability things
###############################################################################

predmaps_c = pickle.load( open( Fig_dir2 + 'Ccor_all.pkl', "rb" ) )
#Long_wks = pickle.load( open( Fig_dir2 + 'Longevity_wks_all.pkl', "rb" ) )
Long_wks_c = pickle.load( open( Fig_dir2 + 'Longevity_wks_all_c.pkl', "rb" ) )
...
probamaps = np.asarray(pickle.load( open( probafiles[M], "rb" ) ))
print (type(probamaps))
...
for num in range(len(prediction_scenes)):
	...
	proba = np.mean(probamaps[Season_idx], axis = 0)
	probastd = np.std(probamaps[Season_idx], axis = 0)

	# Threshold values basics
	counter=0
	zoneseps = np.zeros(proba.shape, dtype = np.uint8)
	for z in range(1,5):
		w = np.where(zones==z)
		#print (sepvals[counter])
		zoneseps[w] = int(100*sepvals[counter])
		counter+=1

	# proba relative to threshold
	proba = 100 * (proba - zoneseps)
	proba[proba >= 0] = proba[proba >= 0] / (100-zoneseps[proba >= 0])
	proba[proba <  0] = 100 - (proba[proba <  0] / zoneseps[proba < 0] * -1)

	# Make seasonal frequency
	SF2 = SF2_c[SF_2_idx]
	SF2[proba == 255] = 0
	SF2[mask !=0] = 0
	...
	######################################################
	# Make Confidence
	dC = (SF2/100 * proba * (100-probastd)/100).astype(np.uint8)
	dC[pred == 0] = 100 - (proba[pred==0] * (100-probastd[pred==0])/100).astype(np.uint8)
	...
	ax.imshow(pred, vmin = 0, vmax = 2)
	ax1.imshow(dC, vmin = 0, vmax = 100)
	plt.show()
	#quit()
...
pickle.dump( PRED, open( Fig_dir2+'PERF/PRED.pkl', "wb" ) )
pickle.dump( CONF, open( Fig_dir2+'PERF/CONF.pkl', "wb" ) )
...
# Longevity in scenes is done
# Now for the longevity in days!
...
axo = plt.subplot2grid((4,1), (0,0), rowspan=1, colspan=1)
axe = plt.subplot2grid((4,1), (1,0), rowspan=1, colspan=1)
axl = plt.subplot2grid((4,1), (2,0), rowspan=1, colspan=1)
axw = plt.subplot2grid((4,1), (3,0), rowspan=1, colspan=1)

axo.set_ylim(0,110)
axe.set_ylim(0,1.1)
axl.set_ylim(0.1,50)
axw.set_ylim(0.1,300)

axo.plot(pred_times, P)
axe.plot(pred_times, C)

axl.plot(pred_times, Longevity * C, 'r')
axl.plot(pred_times, Longevity * (1-C), 'b')

axw.plot(pred_times, Longevity_wks * C, 'r')
axw.plot(pred_times, Longevity_wks * (1-C), 'b')
...
fig = plt.figure(0, facecolor='White',figsize=[13, 20])
axo = plt.subplot2grid((2,1), (0,0), rowspan=1, colspan=1)
axe = plt.subplot2grid((2,1), (1,0), rowspan=1, colspan=1)

axo.set_ylim(0,110)
axe.set_ylim(0,1.1)

axo.plot(pred_times, probamaps[:,1300,370])
axe.plot(pred_times, predmaps[:,1300,370])

plt.show()

quit()
#FTUmaps = pickle.load( open( Fig_dir2 + '/FTU_all.pkl', "rb" ) )
...
for scene in prediction_scenes:
	...
	if scene != modelscene:
		...
		proba = probamaps[counter]

		pred = fn.proba2pred(probamaps[counter],sepvals,zones,mask)
		pred[proba == 255] = 255
		...
		if scene in Model_scenes+Inletscenes+Digiscenes:
		#if scene != '1':
			...
			if scene in Model_scenes:
				...
				# load an additional mask for the aoi
				aoi, pixelWidth, (geotransform, inDs) = ml.ENVI_raster_binary_to_3d_array('/home/willgoodwin/PostDoc/Padova/Data/Basemaps/Inlets_AoI_laguna.bil'); aoi = aoi[:-1]
				kso[aoi < 1] = 255; kso_density[aoi < 1] = 255
				...
				# load an additional mask for the aoi
				aoi, pixelWidth, (geotransform, inDs) = ml.ENVI_raster_binary_to_3d_array('/home/willgoodwin/PostDoc/Padova/Data/Basemaps/Digi_AoI_laguna.bil'); aoi = aoi[:-1]
				kso[aoi < 1] = 255;	kso_density[aoi < 1] = 255
			...
			FTU_TP = FTU[diffmap == 1]
			FTU_TN = FTU[diffmap == 0]
			FTU_FN = FTU[diffmap == -1]
			FTU_FP = FTU[diffmap == -2]
			FTU_T = FTU[diffmap >= 0]
			FTU_F = FTU[diffmap < 0]

			ax_TP.scatter(time, np.percentile(FTU_TP,50), facecolor = fc, marker = mark)
			ax_TN.scatter(time, np.percentile(FTU_TN,50), facecolor = fc, marker = mark)
			ax_FP.scatter(time, np.percentile(FTU_FP,50), facecolor = fc, marker = mark)
			ax_FN.scatter(time, np.percentile(FTU_FN,50), facecolor = fc, marker = mark)


			ax_TP.plot([time,time],[np.percentile(FTU_TP,25),np.percentile(FTU_TP,75)], c = fc)
			ax_TN.plot([time,time],[np.percentile(FTU_TN,25),np.percentile(FTU_TN,75)], c = fc)
			ax_FP.plot([time,time],[np.percentile(FTU_FP,25),np.percentile(FTU_FP,75)], c = fc)
			ax_FN.plot([time,time],[np.percentile(FTU_FN,25),np.percentile(FTU_FN,75)], c = fc)
...
mapz = ax.imshow(np.ma.masked_where(mask!=0, FTU[130]), cmap = plt.cm.autumn_r, vmin = 0, vmax = 10)
...
cbar = plt.colorbar(mapz, cax=axins1, orientation="horizontal", ticks = [0,5,10])

#cbar = plt.colorbar(mapz)
cbar.ax.set_xlabel('Turbidity (FTU)')
...
Figname = 'Performance_bg_m3_md30_znone.png'
...
for M in [1]:
	...
	probamaps = pickle.load( open( probafiles[M], "rb" ) )
	#predmaps = pickle.load( open( predfiles[0], "rb" ) )

	# Define the figure axes
	axS = plt.subplot2grid((3,4), (0,M), rowspan=1, colspan=1)
	axP = plt.subplot2grid((3,4), (1,M), rowspan=1, colspan=1)
	axA = plt.subplot2grid((3,4), (2,M), rowspan=1, colspan=1)

	for ax in [axS,axP,axA]:
		ax.set_ylim(0,1)
		ax.grid(True)
		ax.set_title('Model trained in'+ str(modeltime.year))
		#ax.set_title('Model trained in 2009 (2017-north)')
	if M == 0:
		axS.set_ylabel('S')
		axP.set_ylabel('P')
		axA.set_ylabel('A')
	...
	for scene in prediction_scenes:
		...
		if scene != '1':#modelscene:
			...
			if scene in Model_scenes+Inletscenes+Digiscenes:
				...
				# This is for the other figure
				diffmap, S, P, A = fn.make_diff(pred, kso)
				#fn.make_staple_plot (Fig_dir2+ 'Maps_all/'+str(M)+'_'+txt_time+'_bg_n3_md30_znone.png', counter+1, proba, pred, hs, zones, S, P, A, diffmap, kso_density, sepvals)

			else:
				kso = None; kso_density = None; aoi = None; diffmap = None
				exist_survey = False; S = None; P = None; A = None

			#fn.make_staple_plot (Fig_dir2+ 'Maps_all/'+str(M)+'_'+txt_time+'_bg_z2.png', counter+1, proba, pred, hs, zones, S, P, A, diffmap, kso_density, sepvals)

			#axS.scatter(time, S, marker=mark, facecolor = None, alpha = 0.5, lw = 2*edge_w, edgecolor = 'r')
			#axP.scatter(time, P, marker=mark, facecolor = None, alpha = 0.5, lw = 2*edge_w, edgecolor = 'r')
			#axA.scatter(time, A, marker=mark, facecolor = None, alpha = 0.5, lw = 2*edge_w, edgecolor = 'r')
